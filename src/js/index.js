...
function moverPersonagem(direcao) {
    ...
    if (direcao === 'cima' && posicao.linha > 0) {
        posicao.linha--;
        mudou = true;
    }
    if (direcao === 'baixo' && posicao.linha < linhas - 1) {
        posicao.linha++;
        mudou = true;
    }
    if (direcao === 'esquerda' && posicao.coluna > 0) {
        posicao.coluna--;
        mudou = true;
    }
    if (direcao === 'direita' && posicao.coluna < colunas - 1) {
        posicao.coluna++;
        mudou = true;
    }
...
document.addEventListener('keydown', (e) => {
    ...
    if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
        e.preventDefault(); //impede a rolagem padrao da tela com a tecla up e down
        contadorPassos++;
        atualizarHorario();
    }
...
function atualizarPontuacao() {
    const contador = document.getElementById('score-number');
